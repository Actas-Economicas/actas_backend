...
def get_queries_by_groups(groups):
    ...
    if 'Economia y Desarrollo' in groups or 'admin' in groups:
        options['ALL_ECO'] = {
            'display': 'Generar las solicitudes del Área Economia y Desarrollo',
            'filter': 
                'academic_program__in=2522&academic_program__in=2938&academic_program__in=2951'
        }
        options['PRE_ECO'] = {
            'display': 'Generar las solicitudes del pregrado en Economía',
            'filter': 'academic_program=2522'
        }
        options['POS_ECO'] = {
            'display': 'Generar las solicitudes de posgrados pertenecientes al Área ' + \
                'Economia y Desarrollo',
            'filter': 'academic_program__in=2938&academic_program__in=2951'
        }
    if 'Contabilidad y Finanzas' in groups or 'admin' in groups:
        options['ALL_CON'] = {
            'display': 'Generar las solicitudes del Área Contabilidad y Finanzas',
            'filter': 
                'academic_program__in=2521&academic_program__in=2864'
        }
        options['PRE_CON'] = {
            'display': 'Generar las solicitudes del pregrado en Contaduría Pública',
            'filter': 'academic_program=2521'
        }
        options['POS_CON'] = {
            'display': 'Generar las solicitudes de posgrados pertenecientes al Área ' + \
